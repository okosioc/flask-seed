...
from core.models import DemoProject, DemoProject, DemoProject, DemoProjectDashboard, DemoTask, DemoTask, DemoTeam, DemoTeam, DemoUser
...
def task_edit():
    ...
    #
    demo_users, demo_users_pagination = DemoUser.search({}, projection=['name', 'status', 'roles', 'type', 'email', 'create_time'], sort=[('create_time', -1)])
    current_app.logger.info(f'Preloaded {len(demo_users)} demo users')
    preloads.update({'demo_users': demo_users, 'demo_users_pagination': dict(demo_users_pagination), })
    #
